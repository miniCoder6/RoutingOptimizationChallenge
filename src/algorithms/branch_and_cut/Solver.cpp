...
Solver::Solver(const std::vector<Request> &r, const std::vector<Vehicle> &v, GraphBuilder &gb)
    : requests(r), vehicles(v), graph(gb), checker(gb.nodes, r, v, gb.max_global_capacity, EvaluationMode::PENALTY) {}
...
void Solver::buildInitialSolution(Solution &sol) {
    ...
    for (int i = 0; i < (int)requests.size(); ++i) sorted_reqs[i] = i;
...
bool Solver::insertRequestBest(Solution &sol, int req_idx, int forbidden_veh_id) {
    ...
    for (auto &[veh_id, route] : sol.routes) {
        if (veh_id == forbidden_veh_id) continue;
...
bool Solver::insertRequestBest(Solution &sol, int req_idx) { return insertRequestBest(sol, req_idx, -1); }
...
bool Solver::operatorRelocate(Solution &sol, double threshold) {
    ...
    for (auto const &[vid, r] : sol.routes)
        if (r.size() > 2) nonEmptyVehicles.push_back(vid);

    if (nonEmptyVehicles.empty()) return false;
    ...
    for (int node : src_route)
        if (graph.nodes[node].type == Node::PICKUP) reqs_in_route.push_back(graph.nodes[node].request_id);

    if (reqs_in_route.empty()) return false;
    ...
    for (int node : src_route)
        if (node != p_id && node != d_id) new_src_route.push_back(node);
    ...
    for (auto &[tgt_veh_id, tgt_route] : sol.routes) {
        if (tgt_veh_id == src_veh_id) continue;
...
bool Solver::operatorExchange(Solution &sol, double threshold) {
    ...
    for (auto &[id, r] : sol.routes)
        if (r.size() > 2) active.push_back(id);
    if (active.size() < 2) return false;

    int v1 = active[rand() % active.size()];
    int v2 = active[rand() % active.size()];
    while (v1 == v2) v2 = active[rand() % active.size()];

    auto getReq = [&](const std::vector<int> &r) {
        std::vector<int> reqs;
        for (int n : r)
            if (graph.nodes[n].type == Node::PICKUP) reqs.push_back(graph.nodes[n].request_id);
    ...
    if (pen_v1_old >= 1e15 || pen_v2_old >= 1e15) return false;
    ...
    if (best_v1_route.empty()) return false;
    ...
    if (best_v2_route.empty()) return false;
...
bool Solver::operatorEliminate(Solution &sol) {
    ...
    for (auto const &[vid, route] : sol.routes)
        if (route.size() > 2) candidate_vehicles.push_back(vid);

    if (candidate_vehicles.empty()) return false;
    ...
    for (int node_id : victim_route)
        if (graph.nodes[node_id].type == Node::PICKUP) orphaned_requests.push_back(graph.nodes[node_id].request_id);
...
bool Solver::operator2Opt(Solution &sol, double threshold) {
    ...
    for (auto &[id, r] : sol.routes)
        if (r.size() > 2) active_vehs.push_back(id);
    if (active_vehs.size() < 2) return false;

    int v1_id = active_vehs[rand() % active_vehs.size()];
    int v2_id = active_vehs[rand() % active_vehs.size()];
    while (v1_id == v2_id) v2_id = active_vehs[rand() % active_vehs.size()];
    ...
    auto getEmptyPoints = [&](const std::vector<int> &r, int veh_idx) {
        ...
        for (size_t i = 0; i < r.size() - 1; ++i) {
            ...
            if (load == 0 && i > 0) points.push_back(i);
        }
        return points;
    };
    ...
    if (cuts1.empty() || cuts2.empty()) return false;

    int idx1 = cuts1[rand() % cuts1.size()];
    int idx2 = cuts2[rand() % cuts2.size()];

    std::vector<int> new_r1, new_r2;
    for (int k = 0; k <= idx1; ++k) new_r1.push_back(r1[k]);
    for (int k = idx2 + 1; k < (int)r2.size(); ++k) new_r1.push_back(r2[k]);

    for (int k = 0; k <= idx2; ++k) new_r2.push_back(r2[k]);
    for (int k = idx1 + 1; k < (int)r1.size(); ++k) new_r2.push_back(r1[k]);
    ...
    if (!checker.runEightStepEvaluation(new_r1, idx1_v)) return false;
    pen1_new = checker.getPenalty();

    if (!checker.runEightStepEvaluation(new_r2, idx2_v)) return false;
...
bool Solver::operatorInsertUnserved(Solution &sol) {
    if (sol.unserved_requests.empty()) return false;
