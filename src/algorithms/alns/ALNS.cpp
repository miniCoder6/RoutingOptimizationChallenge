...
std::vector<Route> solveALNS(const std::vector<Employee> &emp, const std::vector<Vehicle> &veh, const Metadata &meta) {
    std::mt19937 rng(7);

    std::vector<Route> initialSol;
    for (const auto &v : veh) initialSol.push_back({v.id, {}});

    greedyRepair(initialSol, emp, veh, meta);

    double initialCost = 0.0;
    for (const auto &r : initialSol) initialCost += routeCost(r, veh[r.vehicleId], emp, meta);
    ...
    enum DestroyOp { RAND_D, LONGEST_D, VEHICLE_D, WORST_D };
    ...
    for (int it = 1; it < tot_it; it++) {
        ...
        int d = selectOperator({dStats[0].weight, dStats[1].weight, dStats[2].weight, dStats[3].weight}, rng);

        int r = selectOperator({rStats[0].weight, rStats[1].weight, rStats[2].weight}, rng);
        ...
        double nextCost = 0.0;
        for (const auto &rt : next) nextCost += routeCost(rt, veh[rt.vehicleId], emp, meta);
