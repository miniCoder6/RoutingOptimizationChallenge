...
void loadMatrix(const std::string &filename, int size) {
    ...
    for (int i = 0; i < size; i++)
        for (int j = 0; j < size; j++) fin >> matrix[i][j];
}
...
double getDistanceFromMatrix(const std::string &a, const std::string &b) { return matrix[convert(a)][convert(b)]; }
...
EvalResult evaluateRoute(const string &v_id, const vector<Step> &route) {
    ...
    for (const auto &step : route) {
        ...
        if (step.type == 1) {  // Pickup
            ...
            if (r.veh_pref != "any" && r.veh_pref != v.category) res.penalty += GAMMA;
        } else {  // Drop
...
Solution initialSolution() {
    ...
    sort(pending.begin(), pending.end(),
         [](const string &a, const string &b) { return requests[a].e_pickup < requests[b].e_pickup; });
...
Solution neighborMove(Solution sol) {
    ...
    for (auto const &entry : sol.routes)
        if (!entry.second.empty()) active_vehs.push_back(entry.first);
    ...
    for (auto s : r_from)
        if (s.req_id != req_to_move) new_r_from.push_back(s);
...
Solution neighborSwap(Solution sol) {
    ...
    for (auto s : sol.routes[v1])
        if (s.req_id != r1) rt1.push_back(s);
    for (auto s : sol.routes[v2])
        if (s.req_id != r2) rt2.push_back(s);
...
int main(int argc, char **argv) {
    ...
    for (const string &vid : veh_ids) {
        ...
        for (auto const &entry : p_times) {
            ...
            outFileVeh << vid << "," << v.category << "," << eid << "," << formatTime(entry.second) << ","
                       << formatTime(d_times[eid]) << endl;
