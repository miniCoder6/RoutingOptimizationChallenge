...
static void backup_route(RouteBackups &backups, int idx, const Route &route) {
    ...
    backups.push_back({idx, route.sequence, route.f1, route.f2, route.viol_cap, route.viol_tw, route.viol_ride,
...
bool neighborhood_move(Solution &solution, int h, DARPInstance &instance, RouteBackups &backups) {
    ...
    for (int k = 0; k < h; ++k) {
        if (non_empty_indices.empty()) break;
        ...
        if (pickup_nodes.empty()) continue;
...
bool neighborhood_swap(Solution &solution, int h, DARPInstance &instance, RouteBackups &backups) {
    ...
    auto extract_random_request_ids = [&instance](Route *route, int count) -> std::vector<int> {
        ...
        if (p_indices.empty()) return req_ids;

        std::shuffle(p_indices.begin(), p_indices.end(), gen);
        if (count > (int)p_indices.size()) count = p_indices.size();
    ...
    for (int req_id : req_ids_1) {
        ...
        if (!(req.compatible_vehicle_types & (1u << r2.vehicle->type_id))) return false;
    }
    for (int req_id : req_ids_2) {
        const Request &req = instance.get_request(req_id);
        if (!(req.compatible_vehicle_types & (1u << r1.vehicle->type_id))) return false;
    }
...
bool neighborhood_chain(Solution &solution, int h, DARPInstance &instance, RouteBackups &backups) {
    ...
    if (non_empty_indices.empty()) return false;
    ...
    for (int step = 0; step < h; ++step) {
        if (current->sequence.empty()) break;
...
void apply_local_search(Solution &solution, DARPInstance &instance) {
    for (auto &route : solution.routes) {
        if (route.sequence.empty() || route.stats_valid) continue;

        double best_route_cost =
            calculate_route_cost_delta(route, instance, solution.alpha, solution.beta, solution.gamma);

        std::vector<int> pickup_nodes;
        for (int node_id : route.sequence) {
            if (instance.nodes[node_id].type == NodeType::PICKUP) pickup_nodes.push_back(node_id);
        }
        ...
        for (int p_node_id : pickup_nodes) {
            ...
            auto result =
                insert_request_best_position(route, req, instance, solution.alpha, solution.beta, solution.gamma);
...
Solution vns1(const Solution &initial_solution, DARPInstance &instance, long long int k_max, int h,
    ...
    if (!current.unassigned_requests.empty()) {
        ...
        for (int req_id : current.unassigned_requests) {
            ...
            for (auto &route : current.routes) {
                if (!(req.compatible_vehicle_types & (1u << route.vehicle->type_id))) continue;
                auto result =
                    insert_request_best_position(route, req, instance, current.alpha, current.beta, current.gamma);
    ...
    while (current_iteration <= k_max) {
        ...
        if (!current.unassigned_requests.empty()) {
            ...
            current.unassigned_requests = std::move(still_unassigned);
            if (reinserted) update_solution_costs(current, instance);
        }
