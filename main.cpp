...
json generate_matrix_file(const std::string &empData,
                          const std::string &vehData, Matrix &outMatrix,
  ...
  try {
    ...
    auto fetch_block = [&](const std::vector<int> &srcBlock,
      ...
      std::string url = "http://router.project-osrm.org/table/v1/driving/" +
                        coordStr.str() + "?sources=" + srcStr.str() +
                        "&destinations=" + dstStr.str() +
                        "&annotations=distance";

      fs::path tmpJson =
          reqDir / ("osrm_block_" + std::to_string(srcBlock.front()) + "_" +
                    std::to_string(dstBlock.front()) + ".json");

      std::string cmd =
          "curl -sS \"" + url + "\" -o \"" + tmpJson.string() + "\"";
      ...
      if (osrm_success) {
        ...
        for (size_t i = 0; i < srcBlock.size(); ++i) {
          for (size_t k = 0; k < dstBlock.size(); ++k) {
            outMatrix[srcBlock[i]][dstBlock[k]] = haversine(
                coords[srcBlock[i]].first, coords[srcBlock[i]].second,
...
// Fixed Solver Logic
SolverResult run_solver(std::string folderName, std::string execName,
                        fs::path reqDir) {
  SolverResult res;

  // 1. Identify paths
  fs::path projectRoot =
      fs::current_path(); // Where the server is running (assumed root)
  fs::path exePath = projectRoot / folderName / execName;
  fs::path runDir =
      reqDir / folderName; // The temp folder for this specific algo

  // 2. Ensure the run directory inside temp exists
  fs::create_directories(runDir);

  // 3. Construct Command
  // Logic: cd into [tmp/req_id/ALNS] -> run [Root/ALNS/main_ALNS] -> pass
  // [tmp/req_id] as arg This ensures output files are created inside
  // tmp/req_id/ALNS
  std::string logFile = "run_log.txt"; // Relative to runDir

  std::string command = "cd \"" + runDir.string() + "\" && " + "\"" +
                        exePath.string() + "\" \"" + reqDir.string() + "\" > " +
                        logFile + " 2>&1";
  ...
  CROW_ROUTE(app, "/upload")
      .methods(crow::HTTPMethod::POST)([](const crow::request &req) {
        ...
        if (empData.empty() || vehData.empty() || metaData.empty() ||
            modeData.empty() || baseData.empty()) {
          return crow::response(400, "Missing one of the 4 CSVs (employees, "
                                     "vehicles, metadata, baseline, mode).");
        }
        ...
        // --- TEMP DIR CLEANUP ---
        // Change 'false' to 'true' if you want folders deleted automatically
        // after the request finishes. Keeping it 'false' helps you debug by
        // inspecting the tmp folder.
        TempDirGuard guard{reqDir, false};

        std::cout << "[Backend] Request ID: " << reqDir.filename().string()
                  << std::endl;
        ...
        for (size_t i = 0; i < 5; i++)
          getline(read_metadata, metadata_line);
        if (metadata_line == "allow_external_maps,FALSE")
          do_haversine = 1;

        // 4. Generate Matrix
        Matrix internal_matrix;
        json matrix_status =
            generate_matrix_file(empData, vehData, internal_matrix, reqDir);

        if (matrix_status["status"] == "error") {

          return crow::response(
              500,
              "Matrix Failed: " + matrix_status["message"].get<std::string>());
        }

        // 5. Run Algorithms
        // Note: Ensure your external programs (main_ALNS, etc.) accept the
        // directory path as the first argument!
        SolverResult alns_res, bac_res, crds_res, hd_res, vns_res, god, mem;

        std::thread t1(
            [&]() { alns_res = run_solver("ALNS", "main_ALNS", reqDir); });
        std::thread t2([&]() {
          bac_res = run_solver("Branch-And-Cut", "main_BAC", reqDir);
        });
        // std::thread t3([&](){ crds_res =
        // run_solver("Clustering-Routing-DP-Solver", "main_crds", reqDir); });
        std::thread t4([&]() {
          hd_res = run_solver("Heterogeneous_DARP", "hetero", reqDir);
        });
        // std::thread t5([&](){ vns_res =
        // run_solver("Variable_Neighbourhood_Search", "main_vns", reqDir); });
        ...
        response["results"]["HD"] = {{"status", hd_res.status},
                                     {"logs", hd_res.logs},
        ...
        response["results"]["GOD"] = {{"status", god.status},
                                      {"logs", god.logs},
        ...
        response["results"]["mem"] = {{"status", mem.status},
                                      {"logs", mem.logs},
