...
void loadMetadata(string filename)
{
    ...
    if (!GLOBAL_CONFIG.priority_extensions.count(2))
        GLOBAL_CONFIG.priority_extensions[2] = 20;
    if (!GLOBAL_CONFIG.priority_extensions.count(3))
        GLOBAL_CONFIG.priority_extensions[3] = 30;
    if (!GLOBAL_CONFIG.priority_extensions.count(4))
        GLOBAL_CONFIG.priority_extensions[4] = 45;
    if (!GLOBAL_CONFIG.priority_extensions.count(5))
        GLOBAL_CONFIG.priority_extensions[5] = 60;
    ...
    pair<vector<pair<int, double>>, double> get_schedule(Vehicle v, vector<Passenger> &cluster)
    {
        ...
        for (int i = 1; i < NODES_local; i++)
        {
            ...
            for (auto &p : cluster)
                min_pref = min(min_pref, p.capacity_pref);
            if ((int)cluster.size() > min_pref)
                cap_v += (double)((int)cluster.size() - min_pref);
            if ((int)cluster.size() > v.capacity)
                cap_v += (double)((int)cluster.size() - v.capacity);
            for (auto &p : cluster)
                if (p.pref_premium && !v.is_premium)
                    cap_v += 10000.0;

            double penalized = base_score + GLOBAL_CONFIG.alpha * ride_v + GLOBAL_CONFIG.beta * tw_v + GLOBAL_CONFIG.gamma * cap_v;
...
{
    if (a.capacity_pref == 1 || b.capacity_pref == 1)
        return 1e9;
    if (!are_compatible_vehicle_pref(a.vehicle_pref, b.vehicle_pref))
        return 1e9;
...
int main(int argc, char **argv)
{
    ...
    while (true)
    {
        ...
        for (size_t i = 0; i < current_clusters.size(); i++)
        {
            ...
            for (int u : current_clusters[i])
                for (int vv : current_clusters[i])
                    diam = max(diam, s_matrix[u][vv]);
            if (diam > max_diam)
            {
                max_diam = diam;
                target_idx = i;
        ...
        for (auto &c : current_clusters)
        {
            int min_pref = 3;
            for (int pid : c)
                min_pref = min(min_pref, passengers[pid].capacity_pref);
            if ((int)c.size() > min_pref)
                ok = false;
        }
    ...
    for (size_t i = 0; i < best_config.size(); i++)
    {
        ...
        if (assign_group(cluster_passengers))
            continue;
        ...
        if (cluster_passengers.size() >= 4)
        {
            ...
            if (assign_group(g1) && assign_group(g2))
                continue;
        }
